...
export default function Home() {
  ...
  const sendSignal = async (button: String) => {
    // TODO: User feedback
    try {
      const res = await axios.post("/api/remote", {command: `${button}${String.fromCharCode(3)}`})
      console.log(res)
    } catch (err) {
      console.log(err)
    }
  ...
  return (
    <div>
      <main>
        <div className={"border-4 border-black p-3 select-none w-screen max-w-md"}>
          <div className={"grid grid-cols-2 gap-2"}>
            <div className={"bg-black flex justify-between rounded-lg"}>
          ...
          <div className={"bg-black flex justify-around rounded-lg mt-2 flex-wrap"}>
            {/* Row 1 */}
